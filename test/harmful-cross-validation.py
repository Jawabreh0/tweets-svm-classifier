...
def highlight_and_count_discrepancies(df, output_excel):
    ...
    # Append header
    ws.append([col for col in df.columns])  
...
tweets_df = pd.read_csv('./dataset/testing_dataset/testing-harmful.csv')
...
if 'ground_truth' not in tweets_df.columns:
    tweets_df['ground_truth'] = [0] * len(tweets_df)

# Highlight discrepancies and count them
output_path = './test/cross-validation-harmful.xlsx'
discrepancy_count = highlight_and_count_discrepancies(tweets_df, output_path)

print(f"Tweet classification complete.Total discrepancies: {discrepancy_count}")
